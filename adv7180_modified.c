...
static unsigned int adv7280_lbias_settings[][3] = {
	[ADV7182_INPUT_TYPE_CVBS] = { 0xC7, 0x4E, 0x80 },		//Was 0xCD, 0x4E, 0x80
	[ADV7182_INPUT_TYPE_DIFF_CVBS] = { 0xC0, 0x4E, 0x80 },
	[ADV7182_INPUT_TYPE_SVIDEO] = { 0x0B, 0xCE, 0x80 },
	[ADV7182_INPUT_TYPE_YPBPR] = { 0x0B, 0x4E, 0xC0 },
};
...
static int init_device(struct adv7180_state *state)
{
	...
	/* DJR Edited for Manual fixed Luma and Chroma Gain */
	ret = adv7180_write(state, 0x002C, 0x00);
	if (ret < 0)
		goto out_unlock;
		
	ret = adv7180_write(state, 0x002D, 0xF4);
	if (ret < 0)
		goto out_unlock;
		
	ret = adv7180_write(state, 0x002E, 0x00);
	if (ret < 0)
		goto out_unlock;
		
	ret = adv7180_write(state, 0x002F, 0xF4);
	if (ret < 0)
		goto out_unlock;
		
	ret = adv7180_write(state, 0x0030, 0x00);
	if (ret < 0)
		goto out_unlock;

	//switch on ace
	ret = adv7180_write(state, 0x400e, 0x40);
	if (ret < 0)
		goto out_unlock;
	ret = adv7180_write(state, 0x4080, 0x80);
	if (ret < 0)
		goto out_unlock;
	ret = adv7180_write(state, 0x400e, 0x00);
	if (ret < 0)
		goto out_unlock;
	
	// switch on colour bars instead of blue screen
	ret = adv7180_write(state, 0x0014, 0x11);
	if (ret < 0)
		goto out_unlock;
